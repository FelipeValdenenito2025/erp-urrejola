      ...
      {loading ? (
        ...
        solicitudes.length === 0 ? (
          ...
          <div style={{ background:'white', borderRadius:'12px', border:'1px solid #eee', overflow:'hidden' }}>
            ...
            {(verMasHistorial ? solicitudes : solicitudes.slice(0, 8)).map((s, idx) => {
          ...
          {solicitudes.length > 8 && (
            <div style={{ padding:'12px 16px', background:'#f8f9fa', borderTop:'1px solid #e9ecef', textAlign:'center' as const }}>
              <button onClick={() => setVerMasHistorial(!verMasHistorial)}
                style={{ background:'none', border:'none', color:'#003366', cursor:'pointer', fontSize:'13px', fontWeight:'600', textDecoration:'underline' }}>
                {verMasHistorial ? '▲ Contraer' : `▼ Ver más (${solicitudes.length - 8} registros más)`}
              </button>
            </div>
          )}
        </div>
